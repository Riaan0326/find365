...
export default function Header({ onWelcomeClick, onDisclaimerClick, onBackToSelection, onTermsClick, onPrivacyClick }: HeaderProps) {
  ...
  return (
    <header className="header" style={{
      backgroundImage: 'url("https://res.cloudinary.com/duqmfy7z7/image/upload/v1765031056/letxerco3ghhsdwomvir.png")',
        ...
        <div className="header-content">
          <div className="logo">
            ...
            <div style={{ display: 'inline-block', verticalAlign: 'middle' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#ffffff', lineHeight: '1' }}>
                Rides<sup style={{ fontSize: '0.5rem' }}>911</sup>
              </div>
          ...
          <button 
            className="hamburger" 
            style={{ color: 'white', backgroundColor: 'transparent', border: 'none', fontSize: '1.5rem' }}
            onClick={() => setShowMobileMenu(!showMobileMenu)}
          >
            <i className="fas fa-bars" style={{ color: 'white', fontSize: '1.2rem' }}></i>
          </button>
          <nav className={`nav ${showMobileMenu ? 'nav-open' : ''}`}>
            <div className="legal-dropdown" style={{ position: 'relative', display: 'inline-block' }}>
              <a href="#legal" onClick={(e) => { e.preventDefault(); setShowLegalDropdown(!showLegalDropdown); }} style={{ color: 'white', cursor: 'pointer' }}>Legal</a>
              {showLegalDropdown && (
                <div style={{
                  position: 'absolute',
                  top: '100%',
                  ...
                  <a href="#terms" onClick={(e) => { e.preventDefault(); onTermsClick?.(); setShowLegalDropdown(false); setShowMobileMenu(false); }} 
                     style={{ display: 'block', padding: '0.5rem 1rem', color: 'white', textDecoration: 'none', borderBottom: '1px solid #4b5563', fontWeight: 'normal' }}
                     onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#4b5563'}
                     onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                    Terms & Conditions
                  </a>
                  <a href="#privacy" onClick={(e) => { e.preventDefault(); onPrivacyClick?.(); setShowLegalDropdown(false); setShowMobileMenu(false); }} 
            ...
            <div className="contact-dropdown" style={{ position: 'relative', display: 'inline-block' }}>
              <a href="#contact" onClick={(e) => { e.preventDefault(); setShowContactDropdown(!showContactDropdown); }} style={{ color: 'white', cursor: 'pointer' }}>Contact</a>
              {showContactDropdown && (
                <div style={{
                  position: 'absolute',
                  top: '100%',
                  ...
                  <a href="mailto:find365@mail.com" onClick={() => { setShowContactDropdown(false); setShowMobileMenu(false); }} 
                     style={{ display: 'block', padding: '0.5rem 1rem', color: 'white', textDecoration: 'none', borderBottom: '1px solid #4b5563', fontWeight: 'normal' }}
                     onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#4b5563'}
                     onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                    find365@mail.com
